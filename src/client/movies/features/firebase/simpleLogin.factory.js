...
angular.module('firebase.simpleLogin', ['firebase', 'firebase.utils', 'firebase.changeEmail'])

  // a simple wrapper on simpleLogin.getUser() that rejects the promise
  // if the user does not exists (i.e. makes user required)
  .factory('requireUser', requireUser)

  .factory('simpleLogin', simpleLogin)

  .factory('createProfile', createProfile);

    //////////////////////////////////

    requireUser.$inject = ['simpleLogin', '$q'];
    function requireUser (simpleLogin, $q) {
        return function() {
    ...
    simpleLogin.$inject = ['$firebaseAuth', 'fbutil', 'createProfile', 'changeEmail'];
    function simpleLogin($firebaseAuth, fbutil, createProfile, changeEmail) {
    ...
    createProfile.$inject = ['fbutil', '$q', '$timeout'];
    function createProfile(fbutil, $q, $timeout) {
